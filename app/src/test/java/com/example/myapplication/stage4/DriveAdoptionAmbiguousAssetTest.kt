...
class DriveAdoptionAmbiguousAssetTest {
    @Test fun ordinaryPhotoAdoptionControl() = exercise(Fault.NONE)
    @Test fun committedAssetWithLostResponseIsReconciled() = exercise(Fault.LOST)
    @Test fun committedAssetWithCloseFailureIsReconciled() = exercise(Fault.CLOSE)
    @Test fun reconciledAssetRollsBackWithItsNewEtagWhenFolderFails() = exercise(Fault.LOST, failFolder = true)
    @Test fun uncommittedAssetFailureDoesNotGuessSuccess() = exercise(Fault.BEFORE_COMMIT)
    @Test fun explicitAssetPreconditionFailureDoesNotReplay() = exercise(Fault.PRECONDITION)
    @Test fun externalAssetOwnerIsNeverOverwrittenAfterLostResponse() = exercise(Fault.EXTERNAL_OWNER)
    @Test fun changedAssetContentCannotBeBlessedByMatchingOwnershipTags() = exercise(Fault.EXTERNAL_CONTENT)
    @Test fun movedAssetCannotBeBlessedByMatchingOwnershipTags() = exercise(Fault.EXTERNAL_PARENT)

    @Test fun committedServerFailureIsReconciledWithoutReplayingPut() = exercise(Fault.HTTP_COMMITTED)
    @Test fun uncommittedServerFailureDoesNotGuessSuccess() = exercise(Fault.HTTP_UNCOMMITTED)
    @Test fun rollbackCannotOverwriteAnEditAfterSuccessfulReadback() = exercise(Fault.LOST,
        failFolder = true, externalBeforeRollback = true)

    @Test fun malformedCommittedAcknowledgementRequiresVerifiedReadback() = exercise(Fault.MALFORMED_ACK)
    @Test fun successfulReplyCannotBlessAnotherAssetOwner() = exercise(Fault.EXTERNAL_ACK_SCOPE)
    ...
    private fun exercise(fault: Fault, failFolder: Boolean = false, externalBeforeRollback: Boolean = false) = runBlocking {
        ...
        fun metadata(id: String): String = JsonObject().apply {
            ...
            add("parents", JsonArray().apply { add(if (id == "folder-1") "root" else if (id == "asset-1") assetParent else "folder-1") })
                ...
                override fun execute(): LowLevelHttpResponse {
                    ...
                    if (id == "folder-1" && failFolder) {
                        if (externalBeforeRollback) {
                            properties.getValue("asset-1")[SYNC_DOCUMENT_ID_APP_PROPERTY] = "external-owner"
                            revisions["asset-1"] = 3
                        }
                        return json("{}", 412)
                    }
                    ...
                    if (id == "asset-1" && assetAttempts == 1) {
                        when (fault) {
                            ...
                            Fault.EXTERNAL_CONTENT -> { assetOutputHash = "0".repeat(64); revisions[id] = 3; throw IOException("synthetic lost response") }
                            Fault.EXTERNAL_PARENT -> { assetParent = "external-folder"; revisions[id] = 3; throw IOException("synthetic lost response") }
        ...
        val root = Files.createTempDirectory("stage9b-adoption-ambiguous-asset")
        ...
        try {
            ...
            if (!failFolder && fault in setOf(Fault.NONE, Fault.LOST, Fault.CLOSE, Fault.HTTP_COMMITTED, Fault.MALFORMED_ACK)) {
                ...
                assertTrue("ambiguous or conflicting ownership must fail closed", result is AdoptionResult.Rejected)
                assertEquals(oldId.value, properties.getValue("folder-1")[SYNC_DOCUMENT_ID_APP_PROPERTY])
                assertArrayEquals("rollback must retain original manifest", originalBytes, manifestBytes)
                if (failFolder) {
                    if (externalBeforeRollback) assertEquals("external-owner", properties.getValue("asset-1")[SYNC_DOCUMENT_ID_APP_PROPERTY])
                    else assertEquals(originalAssetProperties, properties.getValue("asset-1"))
                    assertEquals(listOf("\"asset-1-e1\"", "\"asset-1-e2\""), writes.filter { it.first == "asset-1" }.map { it.second })
                    assertTrue(writes.any { it.first == "folder-1" })
                } else {
                    assertEquals(1, assetAttempts)
                    assertFalse(writes.any { it.first == "folder-1" })
                    ...
                    if (fault in setOf(Fault.EXTERNAL_OWNER, Fault.EXTERNAL_ACK_SCOPE)) assertEquals("external-owner", properties.getValue("asset-1")[SYNC_DOCUMENT_ID_APP_PROPERTY])
                    if (fault == Fault.EXTERNAL_CONTENT) assertEquals("0".repeat(64), assetOutputHash)
                    if (fault == Fault.EXTERNAL_PARENT) assertEquals("external-folder", assetParent)
                }
            }
        } finally {
